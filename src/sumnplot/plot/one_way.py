...
def plot_summarised_variable(
    ...
    if observed is not None:
        ...
        ax2.plot(
            summary_df.loc[:, observed].reset_index(drop=True).dropna().index,
    ...
    if fitted is not None:
        ...
        ax2.plot(
            summary_df.loc[:, fitted].reset_index(drop=True).dropna().index,
    ...
    if fitted2 is not None:
        ...
        ax2.plot(
            summary_df.loc[:, fitted2].reset_index(drop=True).dropna().index,
    ...
    if fitted3 is not None:
        ...
        ax2.plot(
            summary_df.loc[:, fitted3].reset_index(drop=True).dropna().index,
    ...
    if fitted4 is not None:
        ...
        ax2.plot(
            summary_df.loc[:, fitted4].reset_index(drop=True).dropna().index,
    ...
    if legend:
        ...
        if (
            (not observed is None)
            or (not fitted is None)
            or (not fitted2 is None)
            or (not fitted3 is None)
            or (not fitted4 is None)
        ):

            plt.legend(bbox_to_anchor=(1.05, 0.94), loc=2, borderaxespad=0.0)
